...
const plugins = [{ name: "Format", continueShown: true }, { name: "Text compression", continueShown: true }, { name: "User preference", continueShown: false }]

const failedLinks: string[] = []

links.forEach((link) => {
	plugins.forEach((selectedPlugin) => {
		test(`Can analyze plugin: ${selectedPlugin.name} on: ${link}`, async ({ page, extensionId }) => {
			test.skip(failedLinks.some(element => element === link), `Skipping: Test previously failed on ${link}`)
			...
			if (selectedPlugin.continueShown) {
				...
				await continueButton.waitFor({ state: "visible", timeout: 100 });
			...
			const resultBox = await popup.locator(
